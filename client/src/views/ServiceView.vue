            ...
            <form v-if="showTask">
                ...
                <div>
                    <b-dropdown text="Assign to:" variant="assignedTo" class="m-2">
                        <b-dropdown-item @click="setToPhoto" href="#">Top chef</b-dropdown-item>
                    </b-dropdown>
                </div>
                <div>
    ...
    methods: {
        async getAcceptedReq() {
            try {
                ...
                this.acceptedReq = data.acc_requests.filter((report) => report.selectedDepartment === "Service");
            } catch (error) {
        ...
        async getDeniedReq() {
            try {
                ...
                const data = await response.json();
                this.deniedReq = data.den_requests.filter((report) => report.selectedDepartment === "Service");
            } catch (error) {
                console.error('Error fetching finance reports:', error);
            }
        },
        async getFinishedTasks() {
            try {
                ...
                const data = await response.json();
                this.finishedTasks = data.serv_tasks.filter((report) => report.comment !== "");
            } catch (error) {
                console.error('Error fetching finance reports:', error);
            }
        },
        ...
        saveTask(data) {
            const fetchResult = fetch('http://127.0.0.1:6002/add_service_task', {
                method: "POST",
                body: JSON.stringify(data, null, 2),
                headers: {
                    'Content-Type': 'application/json'
